...
from lib.models.lmtrack.utils import combine_tokens, recover_tokens
...
class BaseBackbone(nn.Module):
    ...
    def forward(self, x, z):
        ...
        attn_list = []
        for blk in self.blocks:
            x, attn = blk(torch.cat([x, z], dim=1), x, return_attention=True)
            attn_list.append(attn)

        return self.norm(x), x, torch.cat(attn_list, dim=1).mean(dim=1)
